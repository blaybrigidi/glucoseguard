        ...
        const fetchPatients = async () => {
            try {
                ...
                const mapped = response.map(p => ({
                    ...
                    avatar: p.name ? p.name.split(' ').map(n => n[0]).join('').toUpperCase() : '??',
                }));

                // Detect pending → accepted transitions and show toast
                mapped.forEach(p => {
                    const prev = prevStatusMap.current[p.id];
                    if (prev === 'pending' && p.assignmentStatus === 'accepted') {
                        toast.success(`${p.name} accepted your request`, {
                            description: 'They have been moved to your active patients list.',
                            duration: 6000,
                        });
                    }
                    prevStatusMap.current[p.id] = p.assignmentStatus;
                });
